...
public class FollowController {
    ...
    public ResponseEntity<Page<FollowResponse>> getAllFollower(@PathVariable("id") Long id, @RequestBody PageInfo pageInfo){
        PageRequest pageRequest = PageRequest.of(pageInfo.getIndex(), pageInfo.getSize());
        return ResponseEntity.ok(followService.getAllFollowerByUserId(id,pageRequest));
    }

    @PostMapping("/follower/me")
    public ResponseEntity<Page<FollowResponse>> getAllFollower(@RequestBody PageInfo pageInfo){
        PageRequest pageRequest = PageRequest.of(pageInfo.getIndex(), pageInfo.getSize());
        return ResponseEntity.ok(followService.getAllFollowerByUserId(profile.getId(),pageRequest));
    }

    @PostMapping("/following/{id}")
    public ResponseEntity<Page<FollowResponse>> getAllFollowing(@PathVariable("id") Long id,@RequestBody PageInfo pageInfo){
        PageRequest pageRequest = PageRequest.of(pageInfo.getIndex(), pageInfo.getSize());
        return ResponseEntity.ok(followService.getAllFollowingByUserId(id,pageRequest));
    }

    @PostMapping("/following/me")
    public ResponseEntity<Page<FollowResponse>> getAllFollowing(@RequestBody PageInfo pageInfo){
        PageRequest pageRequest = PageRequest.of(pageInfo.getIndex(), pageInfo.getSize());
        return ResponseEntity.ok(followService.getAllFollowingByUserId(profile.getId(),pageRequest));
    }
